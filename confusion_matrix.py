...
x = Conv2D(64, (3, 3), activation='relu',strides=(2,2))(input_image)
...
x = Conv2D(256, (3, 3), activation='relu')(input_image2)
# x=BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, beta_initializer='zeros', gamma_initializer='ones', moving_mean_initializer='zeros', moving_variance_initializer='ones', beta_regularizer=None, gamma_regularizer=None, beta_constraint=None, gamma_constraint=None)(x)
# x = Conv2D(64, (3, 3), activation='relu')(x)
# x = AveragePooling2D((2,2))(x)
x = Conv2D(512, (3, 3), activation='relu',strides=(2,2))(x)
...
x = Capsule(32, 16, 3, True)(x)   
capsule = Capsule(2, 8, 3, True)(x)
output2 = Lambda(lambda z: K.sqrt(K.sum(K.square(z), 2)))(capsule)
...
model.load_weights('weights-improvement-model_1-01.h5')
model2.load_weights('weights-improvement-model_2-02.h5')
...
for i in range(len(x_test)):
    ...
    elif predict[i]==y_test[i] and y_test[i]==1:
        true_negative=true_negative+1
    elif predict[i]!=y_test[i] and y_test[i]==1:
        false_negative=false_negative+1
    elif predict[i]!=y_test[i] and y_test[i]==0:
...
for i in range(len(x_test)):
    ...
    elif predict2[i]==y_test[i] and y_test[i]==1:
        true_negative2=true_negative2+1
    elif predict2[i]!=y_test[i] and y_test[i]==1:
        false_negative2=false_negative2+1
    elif predict2[i]!=y_test[i] and y_test[i]==0:
        false_positive2=false_positive2+1

print('---CM-2---')
print('TP:',true_positive2)
print('TN',true_negative2)
print('FN',false_negative2)
print('FP',false_positive2)
print('------')
...
accuracy_model_1=summation1/len(x_test)
...
specificity_model_1=summation1/np.count_nonzero(y_test==0)
print('----specificity of model 1-----')
print(specificity_model_1)
print('----')
...
sensitivity_model_1=summation2/np.count_nonzero(y_test==1)
print('----sensitivity of model 1-----')
print(sensitivity_model_1)
print('----')
...
accuracy_model_2=summation1/len(x_test)
...
specificity_model_2=summation1/np.count_nonzero(y_test==0)
print('----specificity of model 2-----')
print(specificity_model_2)
print('----')
...
sensitivity_model_2=summation2/np.count_nonzero(y_test==1)
print('----sensitivity of model 2-----')
print(sensitivity_model_2)
print('----')
...
plt.plot(fpr_before[1], tpr_before[1], color='darkorange',
         lw=lw, label='ROC curve model 1 (area = %0.2f)' % roc_auc_before[1])
plt.plot(fpr_after[1], tpr_after[1], color='green',
         lw=lw, label='ROC curve model 2 (area = %0.2f)' % roc_auc_after[1])
